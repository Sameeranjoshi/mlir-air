...
// `makeLayoutCsl` and `probeLayout` (defined below) are exposed at
// `xilinx::csl` namespace scope so CSLLayoutEmitter.cpp can drive
// --emit-csl-layout through the same code path the unified --emit-csl
// uses. The helpers between here and them keep `static` (file-scope)
// linkage — they are private implementation details of the layout
// builder.
...
LogicalResult emitOneWafer(ModuleOp module,
