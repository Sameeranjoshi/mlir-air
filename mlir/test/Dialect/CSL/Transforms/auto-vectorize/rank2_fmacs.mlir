...
//
// 2D matrix FMA: C[i,j] += A[i,j] * B[i,j]  -> @fmacs on mem4d_dsd (rank-2).
...
// CHECK: csl.builtin_call "fmacs"(%[[DC]], %[[DC]], %[[DA]], %[[DB]])

module {
  csl.wafer @mat_fma {arch = "wse3"} {
    csl.program @pe {
      ...
      csl.func @compute {
        ...
        scf.for %i = %c0 to %M step %c1 {
          scf.for %j = %c0 to %N step %c1 {
            ...
            %vc = memref.load %C[%i, %j] : memref<8x16xf32>
            %m  = arith.mulf %va, %vb : f32
            %s  = arith.addf %m, %vc  : f32
            memref.store %s, %C[%i, %j] : memref<8x16xf32>
          }
        }
        csl.return
      }
