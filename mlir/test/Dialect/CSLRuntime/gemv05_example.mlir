...
func.func @gemv05_example() {
  ...
  // CHECK: %[[PLACED:.*]] = csl_rt.place %[[REGION]] at (0, 0) : (!csl_rt.code_region) -> !csl_rt.code_region
  ...
  %after_d2h_y = csl_rt.memcpy_d2h %after_launch "y_result" from (0, 0) with_size (1, 16) elem_per_pe 16 : (!csl_rt.runtime) -> !csl_rt.runtime
  // CHECK: %[[AFTER_D2H_Y:.*]] = csl_rt.memcpy_d2h %[[AFTER_LAUNCH]] "y_result" from (0, 0) with_size (1, 16) elem_per_pe 16 : (!csl_rt.runtime) -> !csl_rt.runtime
