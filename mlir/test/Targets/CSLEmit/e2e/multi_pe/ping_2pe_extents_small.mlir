...
// streams pipeline (csl.dataflow.put/get extent → csl.get_fab_dsd extent →
...
csl.wafer @ping_2pe_extents_small {arch = "wse3"} {
  csl.program @left_pe {
    ...
    csl.func @compute {
      ...
      csl.dataflow.put @send_ch source(%buf) extent(%n : index) : memref<32xf32>
      csl.return
    }
  ...
  csl.program @right_pe {
    ...
    csl.func @compute {
      %n = arith.constant 32 : index
      csl.dataflow.get @send_ch target(%buf) extent(%n : index) : memref<32xf32>
      csl.return
    }
